...
async function sha256OfFile(p: string): Promise<string | null> {
  try {
    return crypto.createHash("sha256").update(await fs.readFile(p, "utf8")).digest("hex")
  } catch {
    return null
  }
}
...
async function writeAudit(ctx: any, partial: Omit<AuditEntry, "n" | "prevHash" | "hash">): Promise<void> {
  try {
    ...
    const hashInput = JSON.stringify({
      n: base.n, at: base.at, sessionID: base.sessionID, agent: base.agent,
      action: base.action, effect: base.effect, decision: base.decision,
      category: base.category, extra: base.extra, judged: base.judged,
      resources: base.resources, prevHash: base.prevHash,
    })
  ...
  async setup(ctx) {
    ...
    await ctx.permission.hook("evaluate", async (event: any) => {
      try {
        ...
        if (state.denials >= opts.maxDenials) {
          ...
          await writeAudit(ctx, mkAudit(event, "deny", "session_limit", `denials=${state.denials}`))
          return
        }
        if (state.totalActions > opts.maxActions) {
          event.effect = "deny"
          event.message = `${PLUGIN_NAME}: límite de ${opts.maxActions} acciones`
          await writeAudit(ctx, mkAudit(event, "deny", "session_limit", `actions=${state.totalActions}`))
          return
        }
        ...
        if (action === "read" || action === "edit" || action === "write" || action === "apply_patch") {
          for (const r of event.resources as string[]) {
            if (isProtectedPath(r, opts.protectedPaths)) {
              ...
              state.denials++
              await writeAudit(ctx, mkAudit(event, "deny", "self_protection", `hash=${shortHash(r)}`))
              return
            }
          }
        }
        ...
        if (action === "webfetch" || action === "websearch") {
          ...
          for (const r of event.resources as string[]) {
            if (!isTrustedUrl(r, opts.trustedDomains)) {
              try { blockedHost = new URL(r).hostname } catch { blockedHost = "<invalid-url>" }
              break
            }
          }
          if (blockedHost) {
    ...
    await ctx.tool.hook("execute.before", async (input: any) => {
      try {
        ...
        if (tool !== "read" && tool !== "edit" && tool !== "write" && tool !== "apply_patch") return
...
function mkAudit(
  ...
  return {
    ...
    extra,
    judged,
    resources: resources ?? (event.resources as string[] ?? []).slice(0, 5).map((r: string) => redactSecrets(r).slice(0, 200)),
  }
}
